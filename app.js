...
function renderQuestions() {
  ...
  quizForm.innerHTML = checkpoint.questions.map((q, index) => `
    <div class="question">
      <h3>Question ${((currentStage - 1) * 3) + index + 1}: ${q.text}</h3>
      <label>
        Your answer
        <textarea name="${q.id}" required>${answers[q.id] || ''}</textarea>
      </label>
    </div>
  `).join('') + '<button type="submit">✅ Submit answers & reveal next checkpoint</button>';
